...
def train(model, X, y, epochs=10000, tolerance=1e-6):
    ...
    for i in range(epochs):
        ...
        if i % 500 == 0: print(f'Epoch: {i}\tLoss: {loss_value}')
...
# TODO: Figure out what to do with country
...
for train_val_idx, test_idx in CV_o.split(X):
    ...
    for train_idx, val_idx in CV_i.split(X_train_val):
        ...
        for i, model in enumerate(ANNs):
            ...
            net = train(model, X_train, y_train, epochs=5000)
            y_est = net(X_val.to(device)).squeeze().cpu().data.numpy()
    ...
    net = train(model, X_train_val, y_train_val, epochs=5000)
    y_est = net(X_test.to(device)).squeeze().cpu().data.numpy()
